...
export default {
  ...
  validateTransaction: async (req: Request, res: Response) => {
    ...
    try {
      LogHelper.info(`Transaction | user (${req.userId}) initiate validation of a transaction, transactionId: ${transaction.id}`);
      ...
      LogHelper.info(`Transaction | transaction (${transaction.id}) validated by user (${req.userId}) successfully`);

      return res.status(200).json(transaction);
    } catch (error) {
      LogHelper.info(`Transaction | error occurred when validating  transaction (${transaction.id}) generated by user (${req.userId}), error: (${error})`);
