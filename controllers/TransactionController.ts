...
export default {
  ...
  validateTransaction: async (req: Request, res: Response) => {
    ...
    try {
      LogHelper.info(`Transaction | user (${req.userId}) validate a new transaction, transactionId: ${transaction.id}`);
      ...
      LogHelper.info(`Transaction | transaction (${transaction.id}) validated by user (${req.userId}) successfully treated`);
    } catch (error) {
